...
2. For **one rule in depth** (incorrect/correct examples), open the matching file under `rules/<rule-id>.md` (e.g. `/home/mobasir-rc/.gemini/antigravity/skills/react-best-practices/rules/async-parallel.md`, `rules/bundle-barrel-imports.md`). Filenames use the same prefixes as the quick reference (`async-`, `bundle-`, `server-`, …).
...
```bash
cp <repo>/AGENTS.md /home/mobasir-rc/.gemini/antigravity/skills/react-best-practices/react-best-practices.md
rsync -a --delete <repo>/rules/ /home/mobasir-rc/.gemini/antigravity/skills/react-best-practices/rules/
```
